--[[
    ...
    Версия с GUI, удалением старого GUI, отображением позиции игрока и исправлением TextColor.
...
if not successChatEvent then warn("CFX Warning: Ошибка при инициализации ChatEvent: " .. tostring(errChatEvent)) end
...
function sendAction(command, ...)
    ...
    if ChatEvent and ChatEvent.FireServer and typeof(ChatEvent.FireServer) == "function" then
        ...
        -- Предупреждение об отсутствующем ChatEvent уже было выведено при инициализации
        -- Можно добавить дополнительное уведомление в GUI, если нужно
        if _G.CFX_ShowNotification then _G.CFX_ShowNotification("Error: Chat not configured!", Color3.fromRGB(255,100,50)) end
    end
end
...
function processIncomingMessage(senderPlayerName, message)
    ...
    if string.sub(message, 1, #GROUP_PREFIX) == GROUP_PREFIX then
        ...
        if not success or resultOrError == nil then print("CFX: Ошибка декодирования от " .. senderPlayerName .. ": " .. tostring(resultOrError)) return end
...
pcall(function()
    ...
    if TextChatService and TextChatService.MessageReceived then
        table.insert(_G.CFX_GuiConnections, TextChatService.MessageReceived:Connect(function(messageProperties)
            local senderPlayer
            if messageProperties and messageProperties.TextSource and messageProperties.TextSource:IsA("Player") then senderPlayer = messageProperties.TextSource
            elseif messageProperties and messageProperties.MessageSender and messageProperties.MessageSender.Player then senderPlayer = messageProperties.MessageSender.Player end
            if senderPlayer and senderPlayer.Name ~= LocalPlayer.Name then processIncomingMessage(senderPlayer.Name, messageProperties.Text) end
        end))
        print("CFX: Подключен к TextChatService.MessageReceived")
    else print("CFX: TextChatService.MessageReceived не найден.") end
end)
...
function CreateExecutorGUI()
    ...
    if _G.CFX_InternalGuiConnections then
        for _, conn in ipairs(_G.CFX_InternalGuiConnections) do if conn and typeof(conn.Disconnect) == "function" then pcall(function() conn:Disconnect() end) end end
    end
    _G.CFX_InternalGuiConnections = {}

    local screenGui = Instance.new("ScreenGui")
    screenGui.Name = "CFX_ExecutorGui"; screenGui.Parent = playerGui; screenGui.ResetOnSpawn = false; screenGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    local mainFrame = Instance.new("Frame")
    mainFrame.Name = "MainFrame"; mainFrame.Parent = screenGui; mainFrame.BackgroundColor3 = Color3.fromRGB(35, 35, 45); mainFrame.BorderColor3 = Color3.fromRGB(20, 20, 25)
    mainFrame.BorderSizePixel = 1; mainFrame.Active = true; mainFrame.Draggable = true; mainFrame.Size = UDim2.new(0, 320, 0, 450); mainFrame.Position = UDim2.new(0.5, -160, 0.5, -225)
    local titleLabel = Instance.new("TextLabel")
    titleLabel.Name = "Title"; titleLabel.Parent = mainFrame; titleLabel.BackgroundColor3 = Color3.fromRGB(50, 50, 60); titleLabel.Size = UDim2.new(1, 0, 0, 28)
    titleLabel.Font = Enum.Font.SourceSansSemibold; titleLabel.Text = "CFX Part Telepathy"; titleLabel.TextColor3 = Color3.fromRGB(230, 230, 230); titleLabel.TextSize = 16
    local notificationLabel = Instance.new("TextLabel")
    notificationLabel.Name = "NotificationLabel"; notificationLabel.Parent = mainFrame; notificationLabel.Size = UDim2.new(1, -10, 0, 20); notificationLabel.Position = UDim2.new(0, 5, 0, 30)
    notificationLabel.Font = Enum.Font.SourceSans; notificationLabel.Text = "Status: Idle"; notificationLabel.TextColor3 = Color3.fromRGB(180, 180, 180); -- ИСПРАВЛЕНО TextColor на TextColor3
    notificationLabel.TextSize = 12; notificationLabel.TextXAlignment = Enum.TextXAlignment.Left; notificationLabel.BackgroundTransparency = 1
    ...
    _G.CFX_ShowNotification = function(message, color)
        if notificationLabel and notificationLabel.Parent then
            ...
            notificationLabel.TextColor3 = color or Color3.fromRGB(180,180,180) -- ИСПРАВЛЕНО TextColor на TextColor3
            ...
            notifClearTimer = task.delay(3, function()
                 if notificationLabel and notificationLabel.Parent and notificationLabel.Text == message then
                    ...
                    notificationLabel.TextColor3 = Color3.fromRGB(180,180,180) -- ИСПРАВЛЕНО TextColor на TextColor3
                 end
            end)
        end
    end
    ...
    local playerPosTitle = Instance.new("TextLabel"); playerPosTitle.Parent = mainFrame; playerPosTitle.Size = UDim2.new(1, -padding*2, 0, 18); playerPosTitle.Position = UDim2.new(0, padding, 0, currentY)
    playerPosTitle.Font = Enum.Font.SourceSansSemibold; playerPosTitle.Text = "Your Position (X, Y, Z):"; playerPosTitle.TextColor3 = Color3.fromRGB(200, 200, 200)
    playerPosTitle.TextSize = 13; playerPosTitle.TextXAlignment = Enum.TextXAlignment.Left; playerPosTitle.BackgroundTransparency = 1; currentY = currentY + 18
    ...
    local posLabels, posLabelPrefixes = {playerPosXLabel, playerPosYLabel, playerPosZLabel}, {"X: ", "Y: ", "Z: "}
    for i, pLabel in ipairs(posLabels) do
        pLabel.Parent = mainFrame; pLabel.Size = UDim2.new(0.333, -padding, 0, 18); pLabel.Position = UDim2.new(0 + (i-1)*0.333, padding, 0, currentY)
        pLabel.Font = Enum.Font.SourceSans; pLabel.Text = posLabelPrefixes[i] .. "N/A"; pLabel.TextColor3 = Color3.fromRGB(190, 190, 190)
        pLabel.TextSize = 12; pLabel.TextXAlignment = Enum.TextXAlignment.Left; pLabel.BackgroundTransparency = 1
    ...
    for i, fieldName in ipairs(fieldOrder) do
        local label = Instance.new("TextLabel"); label.Parent = mainFrame; label.Size = UDim2.new(0.25, 0, 0, 22); label.Position = UDim2.new(0, padding, 0, currentY)
        label.Font = Enum.Font.SourceSans; label.Text = fieldName .. ":"; label.TextColor3 = Color3.fromRGB(210, 210, 210); label.TextSize = 13; label.TextXAlignment = Enum.TextXAlignment.Left; label.BackgroundTransparency = 1
        local textBox = Instance.new("TextBox"); textBox.Parent = mainFrame; textBox.Size = UDim2.new(0.75, -padding*2, 0, 22); textBox.Position = UDim2.new(0.25, padding, 0, currentY)
        textBox.BackgroundColor3 = Color3.fromRGB(55, 55, 65); textBox.BorderColor3 = Color3.fromRGB(30, 30, 35); textBox.Font = Enum.Font.SourceSans
        textBox.PlaceholderText = defaultValues[fieldName]; textBox.Text = defaultValues[fieldName]; textBox.TextColor3 = Color3.fromRGB(230, 230, 230); textBox.TextSize = 13; textBox.ClearTextOnFocus = false; inputFields[fieldName] = textBox
    ...
    local createButton = Instance.new("TextButton"); createButton.Parent = mainFrame; createButton.BackgroundColor3 = Color3.fromRGB(70, 140, 80); createButton.Size = UDim2.new(1, -padding*2, 0, 30); createButton.Position = UDim2.new(0, padding, 0, currentY)
    createButton.Font = Enum.Font.SourceSansSemibold; createButton.Text = "Create Shared Part"; createButton.TextColor3 = Color3.fromRGB(255, 255, 255); currentY = currentY + 30 + padding
    ...
    local clearButton = Instance.new("TextButton"); clearButton.Parent = mainFrame; clearButton.BackgroundColor3 = Color3.fromRGB(140, 70, 80); clearButton.Size = UDim2.new(1, -padding*2, 0, 30); clearButton.Position = UDim2.new(0, padding, 0, currentY)
    clearButton.Font = Enum.Font.SourceSansSemibold; clearButton.Text = "Clear My Shared Parts"; clearButton.TextColor3 = Color3.fromRGB(255, 255, 255); currentY = currentY + 30 + padding
    ...
    local closeButton = Instance.new("TextButton"); closeButton.Parent = titleLabel; closeButton.Size = UDim2.new(0, 24, 0, 24); closeButton.Position = UDim2.new(1, -28, 0.5, -12)
    closeButton.BackgroundColor3 = Color3.fromRGB(180, 70, 80); closeButton.Font = Enum.Font.SourceSansBold; closeButton.Text = "X"; closeButton.TextColor3 = Color3.fromRGB(255,255,255); closeButton.ZIndex = titleLabel.ZIndex + 1
    ...
    playerPosUpdateConnection = RunService.RenderStepped:Connect(function()
        if LocalPlayer and LocalPlayer.Character and LocalPlayer.Character.PrimaryPart then local pos = LocalPlayer.Character.PrimaryPart.Position
            playerPosXLabel.Text = posLabelPrefixes[1] .. string.format("%.1f", pos.X); playerPosYLabel.Text = posLabelPrefixes[2] .. string.format("%.1f", pos.Y); playerPosZLabel.Text = posLabelPrefixes[3] .. string.format("%.1f", pos.Z)
        else for i, pLabel in ipairs(posLabels) do pLabel.Text = posLabelPrefixes[i] .. "N/A" end end
    end)
    table.insert(_G.CFX_InternalGuiConnections, playerPosUpdateConnection)
    print("CFX: Executor GUI created and enabled."); screenGui.Enabled = true
end
...
print("CFX: Скрипт 'Телепатия' (vGUI, PosDisplay, TextColorFix) загружен.")
